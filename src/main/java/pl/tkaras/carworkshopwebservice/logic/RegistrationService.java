    ...
    public ResponseEntity register(AppUser appUser) {
        ...
        if(token != null){
            try {
                ...
                emailSenderService.send(appUser.getEmail(), buildEmailTemplate(appUser.getFirstname(), link));
        ...
        if(confirmTokenRepo.existsByToken(token)){
            RegistrationConfirmToken registrationConfirmToken =
                    confirmTokenRepo.findByToken(token)
                                    .orElseThrow(() -> new IllegalStateException(String.format("Problem with token ", token)));
